"""
step3_eval.py
Evaluates restored models with Shared-B SVD correction using Triton/CUDA W4A16 paths and reports metrics.
output :
(no output files)
...
class AddSVDCorrection(nn.Module):
    def __init__(
        ...
        role: str,
        is_group: bool,
        group_cache: MiniGroupCache | None,
        alpha_svd: float = 1.0,
    ):
        super().__init__()
        self.inner = inner
...
def main():
    p = argparse.ArgumentParser(
        description="Evaluate CUDA W4A16/Triton with Shared-B optimization for {model}."
    )
    ...
    print(
        f"\n{'='*15} FINAL SUMMARY ({args.model_name} + Original Weights + Triton Asymmetric + Shared-B) {'='*15}"
    )
