"""
w4ax/step4_evaluate_cumulative_fast.py
Runs a faster cumulative restoration evaluation variant for W4Ax Shared-B group selection experiments.
output :
...
def run_single_evaluation(args, groups_to_patch: set[str]):
    ...
    if filtered_bmap:
        model = patch_svd_correction_wrappers(
            model, shared, filtered_bmap, alpha_svd=1.0
        )
...
def main(args):
    ...
    for metric in metrics_to_use:
        ...
        total = len(ranked_groups)
        print(f"\n{'='*20} Metric: {metric} ({total} groups) {'='*20}")


        start_k = max(0, min(total, args.start_from))
        pbar = tqdm(range(start_k, total + 1), desc=f"Metric: {metric}")

        for i in pbar:
            groups_to_patch = set(ranked_groups[:i])
            pbar.set_description(
                f"Metric: {metric} | Restoring {i}/{total} groups (start_from={start_k})"
            )
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Start cumulative restoration from this number of top-ranked groups "
            "(0 = baseline, N = start from N restored groups)."
        ),
    )
    return parser
